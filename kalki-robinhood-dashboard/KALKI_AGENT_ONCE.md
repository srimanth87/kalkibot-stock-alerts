...
5. Check if this is a **close/sell signal**: `signal.raw_json.side === "sell"` or `signal.note` starts with "Close position".
   - If yes, skip to step 6b (sell flow).
   - If no, continue with buy flow below.
6. **Buy flow** — validate the signal:
...
7. (Unified) If broker review returns alerts/order checks on a buy, do not override; dismiss and stop.
10. Call `mark_signal_executed(signal.id)` after successful placement.
11. Sync the latest Robinhood account snapshot back to the dashboard. This is required after every placed order:
...
12. Print a concise result and exit.
